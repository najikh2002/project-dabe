<template>
  ...
  <div class="bg-slate-100 font-inter mt-12">
    ...
    <section class="container mx-auto opacity-0 animate-fadeIn">
      <div class="p-4"><DynamicCarousel :items="carouselItems" :autoplay="3000"" /></div>
    </section>
    ...
    <main class="container mx-auto px-4 sm:px-6 lg:px-8 py-8 opacity-0 animate-fadeIn animation-delay-300">
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        <section class="w-full">
          ...
          <div v-if="fetchError" class="mb-6 p-4 bg-red-100 text-red-700 rounded-md">Terjadi kesalahan saat memuat produk: {{ fetchError }}</div>
          <div v-if="isLoading && !allProducts.length" class="text-center py-10">Memuat produk...</div>
          <div v-if="!isLoading && !fetchError && !allProducts.length" class="text-center py-10">Belum ada produk yang tersedia.</div>

          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 auto-rows-fr">
...
const fetchedProducts = ref<any[]>([]); // Tipe bisa disesuaikan

onMounted(async () => {
  try {
    const response = await fetch(`${config.public.apiBase}/api/produk/list`, {
      method: "GET",
      redirect: "follow",
    });
    ...
    fetchedProducts.value = Array.isArray(result.data) ? result.data : [];
  } catch (error: any) {
    fetchError.value = error.message || "Terjadi kesalahan saat mengambil data.";
