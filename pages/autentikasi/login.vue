      ...
      >
        <div class="flex flex-col gap-3 sm:gap-4">
          ...
          >
            <img
              ...
              alt="Google logo"
              class="mr-2 h-7 w-7 sm:h-8 sm:w-8"
            />Masuk dengan Google
          </button>
        ...
        <form @submit.prevent="loginUser" class="space-y-4 sm:space-y-5">
          ...
          <div class="flex flex-col gap-0.5 sm:gap-1">
            ...
            />
            <NuxtLink
              to="/autentikasi/ganti"
              class="text-right text-xs sm:text-sm text-[#22AB97] hover:underline"
          ...
          <div class="flex flex-col gap-3 sm:gap-4 pt-2">
            ...
            <p class="text-sm text-center">
              ...
              <NuxtLink to="/autentikasi/register" class="text-[#22AB97]"
                >Buat</NuxtLink
              >
            </p>
          </div>
  ...
  try {
    ...
    if (token && userRole) {
      ...
      if (userRole === "petani") {
        ...
        // Asumsikan perazn lain adalah 'pembeli' atau default
        navigateTo("/"); // Arahkan ke halaman utama untuk pembeli
      }
    } else {
      // Jika token atau data pengguna tidak ada dalam respons yang diharapkan
      console.error(
        "Login response missing token or user role. Full API Response:",
        res
      );
      let errMsg =
        "Login berhasil, namun data dari server tidak lengkap atau tidak sesuai format.";
      if (!token)
        errMsg = "Token otentikasi tidak ditemukan dalam respons server.";
      else if (!userRole)
        errMsg = "Peran pengguna tidak dapat diidentifikasi dari data server.";
      error.value = errMsg;
    }
  } catch (err) {
